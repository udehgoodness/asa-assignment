...
- The optional password can be supplied either as the spec'd `password` query parameter or as an `X-Share-Password` header (preferred when both are sent) — the header avoids the password ending up in access logs/browser history the way a query string does. See `docs/findings.md` (VT-13) for the reasoning.
- Known, deliberately deferred gaps (no manual revoke-before-expiry, no dedicated access audit log) are documented with reasoning in `docs/remediation-plan.md`.
